...
### [에러 타입 만들기](src/error/type/type.go)

> 에러 타입은 에러를 확장하여 좀 더 자세한 에러 내용을 저장하기 위해 사용한다. error 타입이 아니더라도 Error 함수를 구현하면 에러로 사용할 수 있다. (error은 Error() 함수를 가진 인터페이스다!)

## [단위 테스트 사용하기](src/unittest/unittest_test.go)

> 소프트웨어는 점점 커지고 복잡해진다. 버그를 잡아내기 점점 어려운 상황이 치닫는다. 이러한 상황을 개선하기 위해 단위 테스트 기법이 등장했다. Go는 자체적으로 단위테스트를 제공한다. 개발 과정 중 자연스럽게 단위 테스트를 구성할 수 있다.
> 또한, 컴파일러에 명령이 내장되어 있어 따로 설치하지 않고 편하게 사용 가능하다. 테스트 함수는 항상 **Test**로 시작해야 한다. 또한, testing 패키지의 T를 항상 매개변수로 받아야 한다.

### [벤치마크 테스트 사용하기](src/unittest/benchmark/benchmark_test.go)

> 벤치마크는 성능을 측정하는 기능이다. 벤치마크를 수행하는 함수는 항상 **Benchmark**로 시작해야 한다. 또한 testing.B를 매개변수로 받아야 한다. 